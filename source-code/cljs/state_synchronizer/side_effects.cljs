...
(defn sensor-did-mount-f
  ...
  ; {}
  [synchronizer-id {:keys [get-trigger-value-f modify-trigger-value-f set-primary-state-f]}]
...
(defn sensor-did-update-f
  ...
  (let [[_ {:keys [get-monitor-value-f modify-trigger-value-f set-primary-state-f]} trigger-value] (reagent/arguments %)]
...
(defn sensor-will-unmount-f
  ...
  ; @param (map) synchronizer-props
  ; {}
  [synchronizer-id {:keys [autoclear? set-primary-state-f]}]
